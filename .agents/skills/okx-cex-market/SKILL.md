...
1. Identify the user intent.
   - Price, candles, trades, or order book → `price-data-commands.md`
   - Indicators → `indicator-commands.md`
   - Funding rate, mark price, price limit, open interest, index ticker → `derivatives-commands.md`
   - Instrument discovery, stock tokens, metals, commodities, forex, bonds → `instrument-commands.md`
   - Cross-skill workflow or multi-step guidance → `workflows.md`

2. Locate or discover the valid instrument ID.
   - Use `okx market instruments --instType ...` or `okx market instruments-by-category --instCategory ...` for non-crypto assets.
   - For options, discover the underlying first with `okx market open-interest --instType OPTION --instId BTC-USD` if needed.
   - Prefer explicit IDs: `BTC-USDT`, `BTC-USDT-SWAP`, `BTC-USDT-250328`, `BTC-USD-250328-95000-C`, `BTC-USD`.

3. Run a read-only market command immediately.
   - No user confirmation is required for market-data lookups.
   - Always add `--json` when the output needs machine-readable or parsed responses.

4. Handle large historical requests carefully.
   - Estimate record count before using `--after`, `--before`, or indicator history.
   - If estimated candles or rows exceed ~500, warn and ask the user before fetching too much data.

5. Confirm the environment when relevant.
   - If `demo=true` is set in the profile, note whether the user is in demo or live mode.
   - Public market data is unchanged by `--demo`/`--live`, but environment context should still be explicit.

6. Route next actions correctly.
   - If the user asks to place/cancel orders, hand off to `okx-cex-trade`.
   - If the user asks about balances or positions, hand off to `okx-cex-portfolio`.
   - If the user wants a bot setup, hand off to `okx-cex-bot`.
